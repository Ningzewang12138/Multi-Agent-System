...
class ToolEnhancedChatService:
    ...
    async def chat_with_tools(
        ...
        if tool_intent and (tool_choice == "required" or tool_choice == "auto"):
            ...
            logger.info("No tool intent detected, using standard LLM response")
            # 让模型正常回复
            response = self.ollama.chat(
                model=model,
                messages=messages,
        ...
        if "read_file" in tool_map:
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    # 确保是文件名（包含扩展名）
                    if '.' in filename:
                        logger.info(f"Detected read_file intent: filename='{filename}'")
                        return {
                            "tool_name": "read_file",
                            "arguments": {
                                "path": filename
        ...
        if "list_directory" in tool_map:
            if any(word in message_lower for word in ["list", "show", "what files", "ls", "dir"]):
                if any(word in message_lower for word in ["file", "directory", "folder", "current", "workspace"]):
                    logger.info("Detected list_directory intent")
                    return {
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    if '.' in filename:  # 确保是文件名
                        logger.info(f"Detected delete_file intent: filename='{filename}'")
                        return {
