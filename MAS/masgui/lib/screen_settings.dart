...
class _ScreenSettingsState extends State<ScreenSettings> {
                      ...
                      child: LayoutBuilder(builder: (context, constraints) {
                        ...
                        var column2 =
                          ...
                          button(
                              ...
                              Icons.cloud_rounded, () {
                            selectionHaptic();
                            _showServerModeDialog();
                          },
                              context: context,
                              description: AppConfig.serverMode == 'multiagent' 
                                ? '\nMulti-Agent Server (${AppConfig.multiAgentServer})'
  ...
  // 服务器模式配置对话框
  void _showServerModeDialog() {
    final urlController = TextEditingController(text: AppConfig.multiAgentServer);
            ...
            content: Column(
              ...
              children: [
                RadioListTile<String>(
                  ...
                  subtitle: const Text('Connect to your PC server with RAG support'),
                  value: 'multiagent',
                  groupValue: AppConfig.serverMode,
                  onChanged: (value) {
                    setDialogState(() {
                      AppConfig.serverMode = value!;
                    });
                  },
                ),
                ...
                RadioListTile<String>(
                  ...
                  subtitle: const Text('Connect directly to Ollama'),
                  value: 'ollama',
                  groupValue: AppConfig.serverMode,
                  onChanged: (value) {
                    setDialogState(() {
                ...
                OutlinedButton.icon(
                  onPressed: () async {
                    if (AppConfig.serverMode == 'multiagent') {
                      AppConfig.multiAgentServer = urlController.text;
                    }
                    
                    final connected = await MultiAgentService().testConnection();
                    if (context.mounted) {
              ...
              FilledButton(
                ...
                onPressed: () {
                  if (AppConfig.serverMode == 'multiagent') {
                    ...
                    prefs?.setString('multiAgentServer', urlController.text);
                  }
                  prefs?.setString('serverMode', AppConfig.serverMode);
